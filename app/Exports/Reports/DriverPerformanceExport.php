...
class DriverPerformanceExport implements FromCollection, ShouldAutoSize, WithEvents, WithHeadings, WithMapping
{
            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                if ($rowCount > 0) {
                    ...
                    $sheet->getStyle("C{$dataStart}:G{$dataEnd}")
                        ->getNumberFormat()
                        ->setFormatCode('#,##0.00');
                    $sheet->getStyle("H{$dataStart}:M{$dataEnd}")
                        ->getNumberFormat()
                        ->setFormatCode('#,##0.00');
                    $sheet->getStyle("N{$dataStart}:N{$dataEnd}")
                        ->getNumberFormat()
                        ->setFormatCode('0.00"%"');
                }
            },
