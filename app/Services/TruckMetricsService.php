...
class TruckMetricsService
{
    ...
    public function metrics(?string $search, ?int $vehicleTypeId, ?string $status = null): array
    {
        ...
        $metricsRow = $query
            ...
            ->selectRaw("SUM(CASE WHEN status IN ('inactive', 'maintenance') THEN 1 ELSE 0 END) as inactive_count")
    ...
    public function applyFilters(Builder $query, ?string $search, ?int $vehicleTypeId, ?string $status = null): Builder
    {
        ...
        if ($status !== null) {
            if ($status === 'inactive') {
                $query->whereIn('status', ['inactive', 'maintenance']);
            } else {
    ...
    private function normalizeStatus(?string $status): ?string
    {
        ...
        return in_array($value, ['active', 'inactive'], true) ? $value : null;
    }
