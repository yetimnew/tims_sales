...
class DriverTruckLifecycleNotification extends Notification implements ChannelAwareNotification, ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $driverCode = $this->payload['driver']['code'] ?? null;
        $truckPlate = $this->payload['truck']['plate'] ?? null;
        $status = $this->payload['status'] ?? null;

        $driverIdentifier = trim(implode(' ', array_filter([$driverName, $driverCode], static fn ($value) => $value !== null)));

        if ($driverIdentifier !== '') {
            $mail->line('Driver: '.$driverIdentifier);
        }
