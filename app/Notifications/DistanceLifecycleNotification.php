...
class DistanceLifecycleNotification extends Notification implements ChannelAwareNotification, ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $from = $this->payload['from_place']['name'] ?? null;
        $to = $this->payload['to_place']['name'] ?? null;
        $distance = $this->payload['distance_km'] ?? null;
        $status = $this->payload['status'] ?? null;

        if ($from !== null && $to !== null) {
            $mail->line(sprintf('Route: %s → %s', $from, $to));
        }

        if ($distance !== null) {
            $mail->line('Distance: '.$distance.' km');
        }
