...
class PlaceLifecycleNotification extends Notification implements ChannelAwareNotification, ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $woredaName = $this->payload['woreda']['name'] ?? null;
        $zoneName = $this->payload['zone']['name'] ?? null;

        if ($name !== null) {
            $mail->line('Place: '.$name);
        }

        if ($woredaName !== null) {
            $mail->line('Woreda: '.$woredaName);
        }
