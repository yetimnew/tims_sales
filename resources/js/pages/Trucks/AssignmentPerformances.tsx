...
export default function TrucksAssignmentPerformances({
    ...
    const breadcrumbs: BreadcrumbItem[] = useMemo(() => [
        ...
        {
            title: 'Driver assignment performances',
            href: `/trucks/${truck.id}/assignments/${assignment.id}/performances`,
        },
    ], [truck.id, truck.plate, assignment.id]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${truck.plate} · Assignment ${assignment.id} · Performances`} />
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                                        {truck.plate} · {assignment.driver?.name ?? assignment.driverid ?? 'Driver assignment'}
                                    </h1>
                                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                                        Performance history for this driver pairing on the truck
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-3">
                            <Badge className={`${assignment.is_attached ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200' : 'bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-200'}`}>
                                {assignment.is_attached ? 'Active assignment' : 'Inactive assignment'}
                            </Badge>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    <Card className="border-0 bg-gradient-to-br from-background to-muted/20 shadow-lg lg:col-span-2">
                        <CardHeader className="border-b bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20">
                            ...
                            <CardDescription>Lifecycle for this truck-driver pairing</CardDescription>
                        </CardHeader>
                        <CardContent className="grid gap-4 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-base font-semibold text-slate-900 dark:text-slate-100">{assignment.driver?.name ?? 'Unknown driver'}</p>
                                {assignment.driver?.driverid && (
                                    <p className="text-xs text-muted-foreground">Driver ID: {assignment.driver.driverid}</p>
                                )}
                            </div>
                            <div>
                    ...
                    <CardHeader className="border-b bg-gradient-to-r from-amber-50 via-orange-50 to-yellow-50 dark:from-amber-950/20 dark:via-orange-950/20 dark:to-yellow-950/20">
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <CardDescription>Operational history for this driver assignment</CardDescription>
                            </div>
