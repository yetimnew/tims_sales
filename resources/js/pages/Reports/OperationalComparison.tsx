...
interface OperationalComparisonRow {
    operation_id: number;
    customer_id: number | null;
    ...
    total_ton_km: number;
    internal_distance_with_cargo: number;
    internal_distance_without_cargo: number;
    internal_distance: number;
    outsource_distance: number;
    total_distance: number;
    internal_fuel_cost: number;
    internal_perdiem: number;
    internal_work_on_going: number;
    internal_other_cost: number;
...
interface OperationalComparisonSummary {
...
interface OperationalComparisonProps {
    filters: Filters;
    rows: OperationalComparisonRow[];
    summary: OperationalComparisonSummary;
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Reports', href: '/reports/operational-comparison' },
    { title: 'Operational Comparison', href: '/reports/operational-comparison' },
];
...
export default function OperationalComparison({
    ...
    const handleToggleOperation = (id: number) => {
        setSelectedOperations((current) =>
            current.includes(id) ? current.filter((operationId) => operationId !== id) : [...current, id],
        );
    };
    ...
    const handleApplyFilters = () => {
        ...
        router.get('/reports/operational-comparison', params, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleReset = () => {
        ...
        setOperationSelectorOpen(false);
        router.get('/reports/operational-comparison', {}, { preserveState: false, preserveScroll: true });
    };
    ...
    const handleExport = (format: 'csv' | 'xlsx' | 'pdf') => {
        ...
        const url = `/reports/operational-comparison/export/${format}${query ? `?${query}` : ''}`;
        window.location.href = url;
    };

    const summaryCards = [
        {
            title: 'Operations Compared',
            value: formatNumber(summary?.operation_count ?? 0),
            helper: 'Unique operations contributing data',
        ...
        },
        {
            ...
            helper: 'Aggregate tonnage delivered',
            icon: <Gauge className="h-3.5 w-3.5 text-indigo-600" />,
            valueClassName: 'text-indigo-600',
        },
        {
            title: 'Profit per Ton-KM',
            value: formatOptionalCurrency(summary?.profit_per_ton_km ?? null),
            helper: 'Net profit generated for each ton-km moved',
            icon: <TrendingUp className="h-3.5 w-3.5 text-emerald-600" />,
            valueClassName: 'text-emerald-600',
        },
        {
            title: 'Revenue per Trip',
            value: formatOptionalCurrency(summary?.revenue_per_trip ?? null),
            helper: 'Average revenue for every completed trip',
            icon: <FileSpreadsheet className="h-3.5 w-3.5 text-teal-600" />,
            valueClassName: 'text-teal-600',
        },
        {
            title: 'Empty Distance Ratio',
            value: formatPercentage(summary?.empty_distance_ratio_percent ?? null),
            helper: 'Share of internal km driven without cargo',
            icon: <Percent className="h-3.5 w-3.5 text-amber-600" />,
            valueClassName: 'text-amber-600',
        },
        {
            title: 'Outsource Trip Share',
            value: formatPercentage(summary?.outsource_trip_share_percent ?? null),
            helper: 'Outsourced trips as a portion of total activity',
            icon: <Fuel className="h-3.5 w-3.5 text-purple-600" />,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Operational Comparison" />
                ...
                <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-4 pb-10 sm:p-6 lg:p-10">
                    <header className="rounded-2xl border border-slate-200 bg-white/95 px-6 py-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-900/70">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="space-y-2">
                                <p className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500 dark:text-slate-400">Transport Economics</p>
                                <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-50">Operational Comparison</h1>
                                <p className="max-w-3xl text-sm text-slate-600 dark:text-slate-300">
                                    Benchmark every operation&apos;s internal and outsourced execution. Track profitability, unit economics, and utilization to pinpoint where transport efficiency can improve.
                                </p>
                            </div>
                    ...
                    <section className="grid gap-6 lg:grid-cols-[360px_1fr]">
                        <Card className="flex h-full flex-col border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                            <CardHeader className="space-y-2">
                                ...
                                <CardDescription className="text-sm">Define the comparison window and focus on specific operations.</CardDescription>
                            </CardHeader>
                        ...
                        <Card className="flex h-full flex-col border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                            <CardHeader className="space-y-2">
                                <CardTitle className="text-lg font-semibold text-slate-900 dark:text-slate-50">Key Economics</CardTitle>
                                <CardDescription className="text-sm">Monitor cost, revenue, and utilization ratios across the fleet.</CardDescription>
                            </CardHeader>
                        ...
                        <section className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
                            <Card className="border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                                <CardHeader className="space-y-2 border-b border-slate-200/60 pb-4 dark:border-slate-700/60">
                                    ...
                                    <CardDescription className="text-sm">Spot customers driving the largest share of throughput.</CardDescription>
                                </CardHeader>
                            ...
                            <Card className="border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                                <CardHeader className="space-y-2 border-b border-slate-200/60 pb-4 dark:border-slate-700/60">
                                    ...
                                    <CardDescription className="text-sm">Monitor how internal versus outsource mix evolves.</CardDescription>
                                </CardHeader>
                                <CardContent className="p-0">
                    ...
                    <Card className="border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                        <CardHeader className="space-y-3 border-b border-slate-200/60 pb-5 dark:border-slate-700/60">
                            <div className="space-y-1">
                                <CardTitle className="text-lg font-semibold text-slate-900 dark:text-slate-50">Operational Economics Detail</CardTitle>
                                <CardDescription className="text-sm">Unit-level profitability and utilization metrics per operation.</CardDescription>
                            </div>
                                ...
                                <Table>
                                    <TableHeader className="bg-slate-50/60 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-900/60 dark:text-slate-400">
                                        <TableRow className="divide-x divide-slate-200/40 dark:divide-slate-800/50">
                                            ...
                                            <TableHead className="whitespace-nowrap text-right">Total Ton-KM</TableHead>
                                            ...
                                            <TableHead className="whitespace-nowrap text-right">Revenue / Ton-KM</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Cost / Ton-KM</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Profit / Ton-KM</TableHead>
                                            ...
                                            <TableHead className="whitespace-nowrap text-right">Tonnage / Trip</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Average Km/Trip</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Total Distance (KM)</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Empty Distance %</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Cost per Km</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Internal Fuel / Km</TableHead>
                                            <TableHead className="whitespace-nowrap text-right">Outsource Cost / Km</TableHead>
                                    ...
                                    <TableBody>
                                        {rows.length === 0 && (
                                            <TableRow>
                                                <TableCell colSpan={27} className="py-6 text-center text-sm text-muted-foreground">
                                        ...
                                        {rows.map((row) => (
                                            <TableRow
                                                key={`${row.operation_id}-${row.operation_code}`}
                                            ...
                                            >
                                                ...
                                                <TableCell className="whitespace-nowrap text-right">{formatDecimal(row.total_ton_km)}</TableCell>
                                                ...
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalCurrency(row.revenue_per_ton_km)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalCurrency(row.cost_per_ton_km)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalCurrency(row.profit_per_ton_km)}</TableCell>
                                                ...
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalDecimal(row.tonnage_per_trip)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalDecimal(row.average_km_per_trip)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatDecimal(row.total_distance)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatPercentage(row.empty_distance_ratio_percent)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalCurrency(row.cost_per_km)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalCurrency(row.internal_fuel_cost_per_km)}</TableCell>
                                                <TableCell className="whitespace-nowrap text-right">{formatOptionalCurrency(row.outsource_cost_per_km)}</TableCell>
                                    ...
                                    <TableFooter>
                                        <TableRow className="divide-x divide-slate-200/40 bg-slate-50/70 font-semibold dark:divide-slate-800/60 dark:bg-slate-900/70">
                                            <TableCell colSpan={2}>
                                                Totals ({formatNumber(summary.operation_count)} operations)
                                            </TableCell>
                                            ...
                                            <TableCell className="text-right">{formatDecimal(summary.total_ton_km)}</TableCell>
                                            ...
                                            <TableCell className="text-right">{formatOptionalCurrency(summary.revenue_per_ton_km)}</TableCell>
                                            <TableCell className="text-right">{formatOptionalCurrency(summary.cost_per_ton_km)}</TableCell>
                                            <TableCell className="text-right">{formatOptionalCurrency(summary.profit_per_ton_km)}</TableCell>
                                            ...
                                            <TableCell className="text-right">{formatOptionalDecimal(summary.tonnage_per_trip)}</TableCell>
                                            <TableCell className="text-right">{formatOptionalDecimal(summary.average_km_per_trip)}</TableCell>
                                            <TableCell className="text-right">{formatDecimal(summary.total_distance)}</TableCell>
                                            <TableCell className="text-right">{formatPercentage(summary.empty_distance_ratio_percent)}</TableCell>
                                            <TableCell className="text-right">{formatOptionalCurrency(summary.cost_per_km)}</TableCell>
                                            <TableCell className="text-right">{formatOptionalCurrency(summary.internal_fuel_cost_per_km)}</TableCell>
                                            <TableCell className="text-right">{formatOptionalCurrency(summary.outsource_cost_per_km)}</TableCell>
