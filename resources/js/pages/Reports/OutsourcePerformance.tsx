...
export default function OutsourcePerformance({
    ...
    const detailBadgeItems = [
        ...
        { key: 'to', label: `To ${filters?.to || '—'}` },
        {
            key: 'vendors',
            label:
                appliedVendorCount > 0
                    ? `${appliedVendorCount} vendor${appliedVendorCount > 1 ? 's' : ''}`
                    : 'All vendors',
        },
        {
            key: 'statuses',
            label:
                appliedStatusCount > 0
                    ? `${appliedStatusCount} status${appliedStatusCount > 1 ? 'es' : ''}`
                    : 'All statuses',
        },
    ];
        ...
        () => [
            {
                ...
                value: isLoading ? <Skeleton className="h-5 w-28" /> : formatCurrency(summary?.total_outsourced_cost ?? 0),
                helper: 'Total paid to outsource partners',
            ...
            },
            {
                ...
                value: isLoading
                    ...
                    : formatOptionalCurrency(summary?.outsourced_cost_per_km, ' / km'),
                helper: 'Average outsource kilometre rate',
                icon: <TrendingUp className="h-3.5 w-3.5" />,
                iconWrapperClassName: 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-200',
                valueClassName: isLoading ? undefined : 'text-indigo-600 dark:text-indigo-200',
            },
            {
                key: 'delta',
                label: 'Δ vs internal',
                value: isLoading
                    ? <Skeleton className="h-5 w-24" />
                    : formatOptionalCurrency(summary?.cost_delta_per_km, ' / km'),
                helper: 'Difference to internal cost baseline',
                icon: <ArrowUpRight className="h-3.5 w-3.5" />,
                iconWrapperClassName: 'bg-rose-50 text-rose-600 dark:bg-rose-500/20 dark:text-rose-200',
                valueClassName: isLoading ? undefined : 'text-rose-600 dark:text-rose-200',
            },
            {
                key: 'completion',
                ...
                valueClassName: isLoading ? undefined : 'text-amber-600 dark:text-amber-200',
            },
            {
                key: 'avg-cost-trip',
                label: 'Avg cost / trip',
                value: isLoading
                    ? <Skeleton className="h-5 w-24" />
                    : formatOptionalCurrency(summary?.average_cost_per_trip),
                helper: 'Spend per completed trip',
                icon: <TrendingDown className="h-3.5 w-3.5" />,
                iconWrapperClassName: 'bg-sky-50 text-sky-600 dark:bg-sky-500/20 dark:text-sky-200',
            },
            {
                key: 'operations-analysed',
                label: 'Operations analysed',
                value: isLoading ? <Skeleton className="h-5 w-20" /> : formatNumber(totals?.vendor_count ?? 0),
                helper: 'Vendors included in this window',
                icon: <LayoutGrid className="h-3.5 w-3.5" />,
                iconWrapperClassName: 'bg-slate-100 text-slate-600 dark:bg-slate-800/60 dark:text-slate-200',
            },
        ],
        [isLoading, summary, totals],
    );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <ReportPageShell>
                <ReportHero
                    eyebrow="Outsource insight"
                    title="Outsource performance"
                    description="Compare outsource hauliers against your internal baseline to watch cost efficiency, reliability, and spend concentration."
                    actions={
                        <>
                ...
                <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
                    <ReportSectionCard
                        title="Filters"
                        description="Adjust the date window and included partners."
                    ...
                    <ReportSectionCard
                        ...
                        description="Key outsourcing metrics for the selected period."
                ...
                <div className="grid gap-6 xl:grid-cols-3">
                    <ReportSectionCard
                        ...
                        description="Internal fleet performance for comparison."
                        contentClassName="space-y-3 p-6 text-sm"
                    >
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-slate-900 dark:text-slate-100">
                                {isLoading ? (
                                    ...
                                    `${formatDecimal(baseline?.distance_km ?? 0)} km`
                                )}
                            </span>
                        </div>
                        <div className="flex items-center justify-between">
                    ...
                    <ReportSectionCard
                        title="Highlights"
                        description="Top performers by key metrics."
                        contentClassName="grid gap-4 p-6 text-sm"
                    >
                        <div className="space-y-2">
                    ...
                    <ReportSectionCard
                        title="Monthly trend"
                        description="Track spend and completion shifts."
                        contentClassName="space-y-3 p-6 text-sm"
                    >
                        {isLoading && (
                            <div className="space-y-3">
                ...
                <ReportSectionCard
                    ...
                    description="Detailed metrics comparing performance, cost efficiency, and reliability across outsource partners."
                            ...
                            <Table>
                                <TableHeader className="bg-slate-50/60 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-900/60 dark:text-slate-400">
                                    <TableRow className="divide-x divide-slate-200/40 dark:divide-slate-800/50">
                                        ...
                                        <TableHead className="whitespace-nowrap text-right">Δ cost / km</TableHead>
                                        <TableHead className="whitespace-nowrap text-right">Δ total</TableHead>
                                        <TableHead className="whitespace-nowrap text-right">Avg cost / trip</TableHead>
                                        <TableHead className="whitespace-nowrap text-right">Avg distance / trip</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {breakdown.length > 0 ? (
                                        breakdown.map((vendor) => (
                                            ...
                                            >
                                                ...
                                                <TableCell className="text-right">
                                                    {formatOptionalCurrency(vendor.cost_per_tonkm, ' / ton-km')}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {formatOptionalCurrency(vendor.cost_delta_per_km, ' / km')}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {formatOptionalCurrency(vendor.total_cost_delta)}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {formatOptionalCurrency(vendor.average_cost_per_trip)}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {formatOptionalDecimal(vendor.average_distance_per_trip, ' km')}
                                                </TableCell>
                                            </TableRow>
                                        ))
                                    ) : (
