...
import { Activity, BarChart3, CircleDollarSign, Download, FileDigit, FileSpreadsheet, FileType2, Filter, GaugeCircle, RefreshCcw, Search, Truck } from 'lucide-react';
...
export default function PerformanceByTruck({ filters, rows = [], summary, trucks }: PerformanceByTruckProps) {
                    ...
                    <section className="grid gap-6 lg:grid-cols-[360px_1fr]">
                        <Card className="flex h-full flex-col border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                            <CardHeader className="space-y-2">
                                ...
                                <CardDescription className="text-sm">
                                    Start with a timeframe, then focus on one truck or a curated group to sharpen the analysis.
                                </CardDescription>
                            </CardHeader>
                                                    ...
                                                    <CommandGroup heading="Fleet">
                                                        ...
                                                        {filteredTruckOptions.map((truckOption) => {
                                                            ...
                                                            return (
                                                                <CommandItem
                                                                    key={truckOption.id}
                                                                    onSelect={() => handleToggleTruck(truckOption.id)}
                        ...
                        <Card className="flex h-full flex-col border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70">
                            <CardHeader className="space-y-2">
                                ...
                                <CardDescription className="text-sm">Stay on top of throughput and financial performance at a glance.</CardDescription>
                            </CardHeader>
                            ...
                            <div className="overflow-x-auto">
                                <Table>
                                    ...
                                    <TableBody>
                                        {rows.length > 0 ? (
                                            rows.map((row) => (
                                                <TableRow key={`${row.truck_id}-${row.plate}`} className="divide-x divide-slate-100 hover:bg-slate-50/70 dark:divide-slate-800/50 dark:hover:bg-slate-900/50">
                                                    ...
                                                    <TableCell className="whitespace-nowrap text-right">{formatDecimal(row.fuel_litres)}</TableCell>
                                                    ...
                                                    <TableCell className="whitespace-nowrap text-right">{formatCurrency(row.work_on_going)}</TableCell>
