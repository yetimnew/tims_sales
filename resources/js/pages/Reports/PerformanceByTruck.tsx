...
export default function PerformanceByTruck({
    filters,
    rows = [],
    summary,
    trucks,
    ...
    const selectedTruckNames = useMemo(
        () => trucks.filter((option) => selectedTrucks.includes(option.id)).map((option) => option.plate),
        [selectedTrucks, trucks],
    );

    const selectedVehicleTypeNames = useMemo(
        () => vehicleTypes.filter((option) => selectedVehicleTypes.includes(option.id)).map((option) => option.name),
    ...
    const filteredTruckOptions = useMemo(() => {
        ...
        return trucks.filter((option) => option.plate.toLowerCase().includes(term));
    }, [truckSearch, trucks]);
    ...
    const filteredVehicleTypeOptions = useMemo(() => {
        ...
        return vehicleTypes.filter((option) => option.name.toLowerCase().includes(term));
    ...
    const handleSelectAllTrucks = () => {
        ...
        setSelectedTrucks(trucks.map((option) => option.id));
    };
    ...
    const handleVehicleTypesSelectAll = () => {
        if (selectedVehicleTypes.length === vehicleTypes.length && vehicleTypes.length > 0) {
            setSelectedVehicleTypes([]);
            return;
        }

        setSelectedVehicleTypes(vehicleTypes.map((option) => option.id));
    };
    ...
    const handleStatusesSelectAll = () => {
        if (selectedStatuses.length === totalStatuses && totalStatuses > 0) {
            setSelectedStatuses([]);
            return;
        }
    ...
    const summaryItems = useMemo(
        () => [
            ...
            },
            {
                ...
                valueClassName: 'text-teal-600 dark:text-teal-200',
            },
        ],
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <ReportPageShell>
                <ReportHero
                    ...
                    description="Monitor utilisation, cost, and profitability by truck. Refine the dataset with precise filters and export polished reports for finance or operations."
                    actions={
                        <>
                            {canExport && (
                                <DropdownMenu>
                ...
                <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
                    <ReportSectionCard
                        title="Filters"
                        description="Focus the performance analysis on the relevant trucks, vehicle types, and statuses."
                    ...
                    >
                        ...
                        <div className="space-y-3 border-t border-slate-200/60 pt-6 dark:border-slate-700/60">
                                ...
                                <PopoverContent className="w-80 p-0" align="start">
                                    <div className="flex items-center justify-between px-3 py-2">
                                        <div className="flex items-center gap-2">
                                            <Button type="button" variant="ghost" size="sm" onClick={handleSelectAllTrucks}>
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                {!noTruckFilter &&
                                    visibleTruckBadges.map((plate) => (
                                        <Badge key={plate} variant="secondary" className="bg-slate-100 text-slate-700 dark:bg-slate-800/70 dark:text-slate-200">
                                            {plate}
                                        </Badge>
                                    ))}
                        ...
                        <div className="space-y-3 border-t border-slate-200/60 pt-6 dark:border-slate-700/60">
                                        ...
                                        <CommandList className="max-h-64">
                                            ...
                                            <CommandGroup heading="Vehicle types">
                                                ...
                                                {filteredVehicleTypeOptions.map((option) => {
                                                    const checked = selectedVehicleTypes.includes(option.id);

                                                    return (
                                                        <CommandItem
                                                            key={option.id}
                                                            onSelect={() => handleToggleVehicleType(option.id)}
                                                            className="flex items-center gap-2"
                                                        >
                                                            <Checkbox checked={checked} />
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                {!noVehicleTypeFilter &&
                                    visibleVehicleTypeBadges.map((name) => (
                                        <Badge key={name} variant="secondary" className="bg-slate-100 text-slate-700 dark:bg-slate-800/70 dark:text-slate-200">
                                            {name}
                                        </Badge>
                                    ))}
                    ...
                    <ReportSectionCard
                        ...
                        description="Aggregate utilisation, spend, and margin for the selected period."
                ...
                <ReportSectionCard
                    title="Truck performance detail"
                    description="Revenue, expense, and utilisation metrics per truck."
