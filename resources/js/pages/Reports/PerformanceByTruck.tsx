...
interface Row { truck_id: number; plate: string; trips: number; tonnage: number; distance_km: number; revenue: number; cost: number; profit: number; margin_percent: number | null }

export default function PerformanceByTruck({ filters, rows }: { filters: { from: string; to: string; truck_id?: number | null }; rows: Row[] }) {
    ...
    const onApply = () => router.get('/reports/performance-by-truck', { from, to }, { preserveState: true });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-3 overflow-hidden rounded-xl p-4">
                ...
                <Card>
                    <CardHeader className="pb-3"><CardTitle className="text-sm">Trucks</CardTitle></CardHeader>
