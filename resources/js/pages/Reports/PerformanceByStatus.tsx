...
export default function PerformanceByStatus({ date, summary = [], latest = [], metrics, statuses = [] }: PerformanceByStatusProps) {
    const [selectedDate, setSelectedDate] = useState(date ?? '');
    const summaryRows = Array.isArray(summary) ? summary : [];
    const latestRows = Array.isArray(latest) ? latest : [];
    const statusOptions = Array.isArray(statuses) ? statuses : [];
    const topStatusName = metrics?.top_status?.status_name ?? null;
    ...
    const formatDateTime = useCallback(
        ...
        [dateTimeFormatter, parseDateValue],
    );
    ...
    const summaryItems = useMemo<ReportSummaryItem[]>(
        ...
        [latestRows.length, metrics, summaryRows.length, latestUpdateDisplay, topStatusDisplay],
    );

    const appliedDateBadge = formatDate(date ?? null);
    const isApplyDisabled = selectedDate === (date ?? '');
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-slate-100/60 dark:bg-slate-900/40">
                <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-4 pb-10 sm:p-6 lg:p-10">
                    <header className="rounded-2xl border border-slate-200 bg-white/95 px-6 py-6 shadow-sm backdrop-blur dark:border-slate-800/70 dark:bg-slate-900/70">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="space-y-2">
                                ...
                                <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-50">Performance by Status</h1>
                                <p className="max-w-3xl text-sm text-slate-600 dark:text-slate-300">
                                    Track how operational statuses evolve throughout the day. Filter by reporting date, monitor the latest changes, and surface the dominant status across the fleet.
                                </p>
                            </div>
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-4">
                                <div className="flex flex-col gap-1">
                                    <span className="text-xs font-medium uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Reporting date</span>
                                    <DatePicker
                                        value={selectedDate || ''}
                                        onChange={(value) => setSelectedDate(value ?? '')}
                                        placeholder="Select a date"
                                        className="w-full min-w-[220px] justify-start text-left h-11 border-slate-300 hover:border-slate-400 focus-visible:border-blue-500 focus-visible:ring-blue-500/20 dark:border-slate-600 dark:hover:border-slate-500"
                                    />
                                </div>
                                <div className="flex items-center gap-2 self-start sm:self-auto">
                                    <Button type="button" variant="secondary" onClick={handleApply} disabled={isApplyDisabled}>
                                        Apply
                                    </Button>
                    ...
                    <div className="grid gap-6 lg:grid-cols-3">
                        <Card className="border border-slate-200 bg-white/95 shadow-sm dark:border-slate-800/70 dark:bg-slate-900/70 lg:col-span-1">
                            <CardHeader className="space-y-3 border-b border-slate-200/60 pb-5 dark:border-slate-700/60">
                                <div className="space-y-1">
                                    <CardTitle className="text-lg font-semibold text-slate-900 dark:text-slate-50">Status distribution</CardTitle>
                                    <CardDescription className="text-sm">Count of vehicles per status with relative share.</CardDescription>
                                </div>
                                <Badge variant="outline">Reporting date · {appliedDateBadge}</Badge>
                            </CardHeader>
                            <CardContent className="p-0">
                                <div className="overflow-x-auto">
                                    <Table>
                                        ...
                                        <TableBody>
                                            {summaryRows.length > 0 ? (
                                                summaryRows.map((row) => {
                                                    const isTopStatus = topStatusName !== null && topStatusName === row.status_name;
                                                        ...
                                                        >
                                                            <TableCell className="whitespace-nowrap font-medium text-slate-900 dark:text-slate-100">
                                                                <div className="flex flex-col">
                                                                    <span>{row.status_name}</span>
                                                                    {isTopStatus ? (
                                                                        <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-300">Top status</span>
                                                                    ) : null}
                                                                </div>
                                                            </TableCell>
                                                            <TableCell className="whitespace-nowrap text-right">{formatInteger(row.count)}</TableCell>
                                                ...
                                                <TableRow>
                                                    <TableCell colSpan={3} className="py-10 text-center text-sm text-muted-foreground">
                                                        No statuses recorded for the selected date.
                                                    </TableCell>
                                                </TableRow>
                                            )}
                                        </TableBody>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {latestRows.length > 0 ? (
                                                ...
                                                <TableRow>
                                                    <TableCell colSpan={6} className="py-10 text-center text-sm text-muted-foreground">
                                                        No recent updates were recorded for this date.
                                                    </TableCell>
                                                </TableRow>
                                            )}
                                        </TableBody>
