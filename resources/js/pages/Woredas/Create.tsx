...
interface WoredaCreateProps {
  zones: Zone[]
}

export default function WoredasCreate({ zones }: WoredaCreateProps) {
  const { toast } = useToast()
  const [frontendErrors, setFrontendErrors] = useState<Record<string, string>>({})
  const { data, setData, post, processing, errors } = useForm<WoredaFormData>({
    name: '',
    code: '',
    zone_id: '',
    description: '',
  ...
  <Link href="/woredas">
        ...
        <h1 className="text-2xl font-bold">Create Woreda</h1>
      </div>

      <Card className="max-w-2xl">
        <CardHeader className="border-b">
          <h2 className="text-lg font-semibold">New Woreda Details</h2>
        </CardHeader>
...
WoredasCreate.layout = (page: React.ReactNode) => <AppLayout children={page} />
