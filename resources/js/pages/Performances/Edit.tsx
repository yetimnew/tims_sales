...
import { useEffect, useCallback, useState, useMemo, type FormEventHandler } from 'react';
...
import { AlertCircle, CheckCircle, Save, ArrowLeft, ClipboardList, Loader2, Pencil } from 'lucide-react';
...
export default function PerformancesEdit({ operations, driverTrucks, places, performance }: PerformancesEditProps) {
    const initialData = useMemo<PerformanceFormData>(() => {
        const existingTonKm = toStringOrEmpty(performance.tonkm);
        const resolvedTonKm = (() => {
            const tonKmNumeric = parseFloat(existingTonKm || '0');
            if (existingTonKm && Number.isFinite(tonKmNumeric) && tonKmNumeric > 0) {
                return tonKmNumeric.toFixed(2);
            }

            const numericDistance = parseFloat(toStringOrEmpty(performance.DistanceWCargo) || '0');
            const numericCargo = parseFloat(toStringOrEmpty(performance.CargoVolumMT) || '0');
            const computed = numericDistance * numericCargo;
        ...
        const normalizedPhase: PerformanceFormData['load_phase'] = performance.load_phase === 'return' || performance.load_phase === 'main' ? performance.load_phase : '';
        const normalizedCompletion: PerformanceFormData['load_completion'] = performance.load_completion === 'partial' || performance.load_completion === 'full'
        ...
        return {
            ...
            DistanceWCargo: toStringOrEmpty(performance.DistanceWCargo),
            DistanceWOCargo: toStringOrEmpty(performance.DistanceWOCargo),
            CargoVolumMT: toStringOrEmpty(performance.CargoVolumMT),
    ...
    const { data, setData, put, processing, errors, setDefaults } = useForm<PerformanceFormData>(initialData);
    ...
    const isDirty = useMemo(() => {
        return (Object.keys(initialData) as (keyof PerformanceFormData)[]).some(key => data[key] !== initialData[key]);
    }, [data, initialData]);

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={performance.trip ? `Edit Performance — ${performance.trip}` : 'Edit Performance'} />
                    ...
                    <CardHeader className="px-6 pb-0">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <CardTitle className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Update Performance</CardTitle>
                                    <CardDescription className="text-sm text-slate-600 dark:text-slate-400">Refine this trip’s operational metrics with quick favorites and inline validation.</CardDescription>
                                    {performance.trip && (
                                        <p className="mt-1 text-xs font-medium text-slate-500 dark:text-slate-400">Editing trip: {performance.trip}</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-slate-200/70 bg-white/80 px-6 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/40">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <Button type="submit" disabled={processing} className="min-w-[160px] bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 text-white shadow-lg transition-all duration-200 hover:from-indigo-700 hover:to-indigo-800 hover:shadow-xl">
                                        {processing ? (<><div className="mr-2 h-4 w-4 animate-spin rounded-full border-b-2 border-white" />Updating...</>) : (<><Save className="mr-2 h-4 w-4" />Update Performance</>)}
                                    </Button>
                                </div>
                            </div>
                        </form>
