...
import { AlertCircle, ArrowLeft, CheckCircle, ClipboardList, Loader2, Pencil } from 'lucide-react';
...
interface Performance {
    ...
    driver_truck_id: string | number | null;
    DateDispach: string;
    orgion_id: string | number | null;
    destination_id: string | number | null;
    DistanceWCargo: string | number | null;
    DistanceWOCargo: string | number | null;
    CargoVolumMT: string | number | null;
    fuelInLitter: string | number | null;
    fuelInBirr: string | number | null;
    perdiem: string | number | null;
    other: string | number | null;
    comment: string | null;
    satus: string;
    is_returned: boolean;
    returned_date: string | null;
    tonkm: string | number | null;
}

interface PerformancesEditProps {
...
const toStringOrEmpty = (value: string | number | null | undefined): string => {
...
const buildInitialForm = (performance: Performance): PerformanceFormData => {
    ...
    const resolvedTonKm = (() => {
        ...
        if (existingTonKm && Number.isFinite(numericTonKm) && numericTonKm > 0) {
            return numericTonKm.toFixed(2);
        }

        const computed = Number.parseFloat(distanceWithCargo || '0') * Number.parseFloat(cargoVolume || '0');
    ...
    const normalizedPhase: PerformanceFormData['load_phase'] =
        performance.load_phase === 'main' || performance.load_phase === 'return' ? performance.load_phase : '';

    const normalizedCompletion: PerformanceFormData['load_completion'] =
        performance.load_completion === 'full' || performance.load_completion === 'partial'
            ? performance.load_completion
            : '';

    const normalizedStatus: 'active' | 'inactive' = performance.satus === 'inactive' ? 'inactive' : 'active';

    return {
        load_phase: normalizedPhase,
        load_completion: normalizedCompletion,
        ...
        DistanceWOCargo: toStringOrEmpty(performance.DistanceWOCargo),
        CargoVolumMT: cargoVolume,
        fuelInLitter: toStringOrEmpty(performance.fuelInLitter),
        fuelInBirr: toStringOrEmpty(performance.fuelInBirr),
        perdiem: toStringOrEmpty(performance.perdiem),
        other: toStringOrEmpty(performance.other),
...
export default function PerformancesEdit({ driverTrucks, performance, places }: PerformancesEditProps) {
    const { t } = useTranslation();
    const initialFormData = useMemo<PerformanceFormData>(() => buildInitialForm(performance), [performance]);
    ...
    const breadcrumbs = useMemo<BreadcrumbItem[]>(
        () => [
            ...
            {
                title: performance.trip || t('performances.form.edit.recordFallback', { id: performance.id }),
                href: `/performances/${performance.id}`,
            },
            { title: t('performances.form.edit.breadcrumb'), href: `/performances/${performance.id}/edit` },
        ],
        [performance, t],
    );

    const { data, setData, setDefaults, put, processing, errors, clearErrors } = useForm<PerformanceFormData>(
        initialFormData,
    );
    ...
    useEffect(() => {
        ...
        setIsDirty(false);
        clearErrors();
    }, [initialFormData, setData, setDefaults, clearErrors]);
    ...
    useEffect(() => {
        ...
        if (errorMessages.length > 0) {
            toast({
                title: t('performances.form.validation.toastTitle'),
        ...
        async (originId: string, destinationId: string, cargoVolume: string) => {
            ...
            try {
                ...
                if (result.found) {
                    ...
                    setDistanceStatus({
                        found: true,
                        message: t('performances.form.distance.autoFilledFromRoute', { value: formattedDistance }),
                    });
                } else {
                    ...
                    setDistanceStatus({
                        found: false,
                        message:
                            result.note ?? t('performances.form.distance.notRegisteredDefaulted'),
                    });
                }
            } catch (error) {
                ...
                setDistanceStatus({
                    found: false,
                    message: t('performances.form.distance.unableResolveDefaulted'),
                });
            } finally {
                setDistanceLoading(false);
            }
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = event => {
        ...
        if (Object.values(clientValidation).some(Boolean)) {
            ...
            toast({
                title: t('performances.form.validation.toastTitle'),
                description: t('performances.form.validation.resolveUpdate'),
                variant: 'destructive',
            });
            return;
        }
        ...
        put(`/performances/${performance.id}`, {
            preserveScroll: true,
            onSuccess: () => {
                const nextDefaults = { ...(data as PerformanceFormData) };
                ...
                setIsDirty(false);
                toast({
                    title: t('performances.form.edit.successTitle'),
                    description: performance.trip
                        ? t('performances.form.edit.successDescription', { trip: performance.trip })
                        : t('performances.form.edit.successDescriptionFallback'),
                });
    ...
    return (
        <FormPageLayout
            title={t('performances.form.edit.title')}
            headTitle={
                performance.trip
                    ? t('performances.form.edit.headTitleWithTrip', { trip: performance.trip })
                    : t('performances.form.edit.headTitle')
            }
            description={t('performances.form.edit.description')}
                ...
                <>
                    <Button variant="ghost" size="sm" asChild>
                        <Link href="/performances">
                            ...
                            {t('performances.form.edit.backToList')}
                        </Link>
                    </Button>
                    {performance.trip && (
                        <Badge variant="outline" className="border-indigo-200 text-indigo-700 dark:border-indigo-900/40 dark:text-indigo-200">
                            {t('performances.form.edit.tripLabel', { trip: performance.trip })}
                        </Badge>
                    )}
                    {isDirty && <UnsavedChangesBadge />}
                    <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                        {t('performances.form.edit.controlBadge')}
                    </Badge>
                </>
            }
        >
            ...
            >
                ...
                >
                    ...
                    <FormField
                        id="load_completion"
                        label={t('performances.form.fields.loadCompletion.label')}
                        required
                        error={getFieldError('load_completion')}
                    >
                        <Select
                    ...
                    <FormField id="driver_truck_id" label={t('performances.form.fields.driverTruck.label')} required error={getFieldError('driver_truck_id')}>
                        <SearchableEntityCombobox
                            ...
                            getLabel={driverTruck =>
                                driverTruck.driver?.name?.trim() || t('performances.form.fields.driverTruck.unknownDriver')
                            }
                                ...
                                if (!selected) {
                                    return (
                                        <span className="text-sm text-muted-foreground">
                                            {t('performances.form.fields.driverTruck.placeholder')}
                                        </span>
                                    );
                                }
                    ...
                    <FormField
                        id="DateDispach"
                        label={t('performances.form.fields.dispatchDate.label')}
                        required
                        error={getFieldError('DateDispach')}
                    >
                        <Input
                            id="DateDispach"
                            type="datetime-local"
                    ...
                    <FormField
                        id="destination_id"
                        label={t('performances.form.fields.destination.label')}
                        required
                    ...
                    left={
                        <>
                            <span className="flex items-center gap-2 text-sm">
                                ...
                                {t('performances.form.actions.requiredUpdate')}
                            </span>
                            ...
                            >
                                {processing ? (
                                    <>
                                        ...
                                        {t('performances.form.actions.updating')}
                                    </>
                                ) : (
                                    <>
                                        <Pencil className="mr-2 h-4 w-4" />
                                        {t('performances.form.actions.update')}
                                    </>
                                )}
                            </Button>
                        </>
