...
export default function DriversAssignmentPerformances({
    ...
    const breadcrumbs: BreadcrumbItem[] = useMemo(() => [
        ...
        {
            title: t('drivers.assignmentPerformances.breadcrumb'),
            href: `/drivers/${driver.id}/assignments/${assignment.id}/performances`,
        },
    ], [assignment.id, driver.id, driver.name, t]);

    const assignmentStatusLabel = assignment.status
        ? `${assignment.status.charAt(0).toUpperCase()}${assignment.status.slice(1)}`
        : assignment.is_attached
            ? t('drivers.assignmentPerformances.status.attached')
            : t('drivers.assignmentPerformances.status.detached');
    ...
    const formatDaysLabel = (value?: number | null, maximumFractionDigits = 0) =>
        formatDays(
            ...
            maximumFractionDigits,
            notAvailableLabel,
            (countLabel) => t('drivers.assignmentPerformances.duration.plural', { count: countLabel }),
            t('drivers.assignmentPerformances.duration.single'),
        );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('drivers.assignmentPerformances.headTitle', { name: driver.name, plate: assignment.plate ?? t('drivers.assignmentPerformances.fallbacks.assignment') })} />
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                            ...
                            >
                                <Link href={`/drivers/${driver.id}`}>
                                    ...
                                    {t('drivers.assignmentPerformances.actions.backToDriver')}
                                </Link>
                            </Button>
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                                        {driver.name} · {assignment.plate ?? t('drivers.assignmentPerformances.fallbacks.assignment')}
                                    </h1>
                                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
                                        {t('drivers.assignmentPerformances.header.description')}
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-3">
                            <Badge className={`${assignment.is_attached ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200' : 'bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-200'}`}>
                                {assignment.is_attached ? t('drivers.assignmentPerformances.status.attached') : t('drivers.assignmentPerformances.status.detached')}
                            </Badge>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    <Card className="border-0 bg-gradient-to-br from-background to-muted/20 shadow-lg lg:col-span-2">
                        <CardHeader className="border-b bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20">
                            <CardTitle className="flex items-center gap-2 text-lg">
                                <Calendar className="h-5 w-5 text-indigo-600" /> {t('drivers.assignmentPerformances.timeline.title')}
                            </CardTitle>
                            <CardDescription>{t('drivers.assignmentPerformances.timeline.description')}</CardDescription>
                        </CardHeader>
                        <CardContent className="grid gap-4 md:grid-cols-2">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">{t('drivers.assignmentPerformances.timeline.driver')}</p>
                                <p className="mt-1 text-base font-semibold text-slate-900 dark:text-slate-100">{driver.name}</p>
                                {driver.driverid && (
                                    <p className="text-xs text-muted-foreground">{t('drivers.assignmentPerformances.timeline.driverId', { value: driver.driverid })}</p>
                                )}
                            </div>
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">{t('drivers.assignmentPerformances.timeline.truck')}</p>
                                <p className="mt-1 text-base font-semibold text-slate-900 dark:text-slate-100">{assignment.plate ?? t('drivers.assignmentPerformances.fallbacks.unknownTruck')}</p>
                                {assignment.truck?.id && (
                                    <p className="text-xs text-muted-foreground">{t('drivers.assignmentPerformances.timeline.truckId', { value: assignment.truck.id })}</p>
                                )}
                            </div>
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">{t('drivers.assignmentPerformances.timeline.assignedOn')}</p>
                                <p className="mt-1 text-sm">{formatDate(assignment.date_recived, longDateFormatter, notAvailableLabel)}</p>
                            </div>
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">{t('drivers.assignmentPerformances.timeline.detachedOn')}</p>
                    ...
                    <Card className="border-0 bg-gradient-to-br from-background to-muted/20 shadow-lg">
                        <CardHeader className="border-b bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-950/20 dark:to-green-950/20">
                            <CardTitle className="flex items-center gap-2 text-lg">
                                <BarChart3 className="h-5 w-5 text-emerald-600" /> {t('drivers.assignmentPerformances.summary.title')}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3 text-sm">
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('drivers.assignmentPerformances.summary.totalRecords')}</span>
                                <span className="font-semibold">{formatNumber(summary.total_records, undefined, notAvailableLabel)}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('drivers.assignmentPerformances.summary.completedTrips')}</span>
                                <span className="font-semibold">{formatNumber(summary.returned_trips, undefined, notAvailableLabel)}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('drivers.assignmentPerformances.summary.activeTrips')}</span>
                                <span className="font-semibold">{formatNumber(summary.active_trips, undefined, notAvailableLabel)}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('drivers.assignmentPerformances.summary.distance')}</span>
                                <span className="font-semibold">{formatKilometers(summary.total_distance_km, 0, notAvailableLabel)}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('drivers.assignmentPerformances.summary.cargoMoved')}</span>
                                <span className="font-semibold">{formatTons(summary.total_cargo_tonnage, 1, notAvailableLabel)}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-muted-foreground">{t('drivers.assignmentPerformances.summary.fuelEfficiency')}</span>
                        ...
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                            <div>
                                <CardTitle className="flex items-center gap-2 text-xl">
                                    <Activity className="h-5 w-5 text-amber-600" /> {t('drivers.assignmentPerformances.records.title')}
                                </CardTitle>
                                <CardDescription>{t('drivers.assignmentPerformances.records.description')}</CardDescription>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="hidden text-sm text-muted-foreground md:block">
                                    {t('drivers.assignmentPerformances.records.dispatchWindow', {
                                ...
                                <Select value={String(perPage)} onValueChange={handlePerPageChange}>
                                    <SelectTrigger className="w-32">
                                        <SelectValue placeholder={t('drivers.assignmentPerformances.records.perPage')} />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {perPageOptions.map(option => (
                                            <SelectItem key={option} value={String(option)}>
                                                {t('drivers.assignmentPerformances.records.showPerPage', { value: option })}
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            <div className="rounded-lg border border-amber-200 bg-amber-50/70 p-4 text-sm dark:border-amber-800 dark:bg-amber-900/20">
                                <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground">
                                    <Navigation className="h-4 w-4" /> {t('drivers.assignmentPerformances.metrics.loadedDistance')}
                                </div>
                            ...
                            <div className="rounded-lg border border-blue-200 bg-blue-50/70 p-4 text-sm dark:border-blue-800 dark:bg-blue-900/20">
                                <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground">
                                    <Gauge className="h-4 w-4" /> {t('drivers.assignmentPerformances.metrics.emptyDistance')}
                                </div>
                            ...
                            <div className="rounded-lg border border-emerald-200 bg-emerald-50/70 p-4 text-sm dark:border-emerald-800 dark:bg-emerald-900/20">
                                <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground">
                                    <Flame className="h-4 w-4" /> {t('drivers.assignmentPerformances.metrics.fuelUsed')}
                                </div>
                            ...
                            <div className="rounded-lg border border-indigo-200 bg-indigo-50/70 p-4 text-sm dark:border-indigo-800 dark:bg-indigo-900/20">
                                <div className="flex items-center gap-2 text-xs uppercase text-muted-foreground">
                                    <BarChart3 className="h-4 w-4" /> {t('drivers.assignmentPerformances.metrics.tonKm')}
                                </div>
                        ...
                        {performances.data.length > 0 ? (
                            <div className="rounded-xl border border-slate-200 dark:border-slate-700">
                                <div className="overflow-x-auto">
                                    ...
                                    <TableHeader className="bg-slate-50/80 dark:bg-slate-900/40">
                                        <TableRow>
                                            <TableHead className="w-[140px]">{t('drivers.assignmentPerformances.table.dispatch')}</TableHead>
                                            <TableHead>{t('drivers.assignmentPerformances.table.route')}</TableHead>
                                            <TableHead className="text-right">{t('drivers.assignmentPerformances.table.distance')}</TableHead>
                                            <TableHead className="text-right">{t('drivers.assignmentPerformances.table.fuel')}</TableHead>
                                            <TableHead className="text-right">{t('drivers.assignmentPerformances.table.cargo')}</TableHead>
                                            <TableHead>{t('drivers.assignmentPerformances.table.status')}</TableHead>
                                            <TableHead className="text-right">{t('drivers.assignmentPerformances.table.tonKm')}</TableHead>
                                            <TableHead className="text-right">{t('drivers.assignmentPerformances.table.actions')}</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                                ...
                                                <TableRow key={row.id} className="bg-white/70 transition hover:bg-blue-50/60 dark:bg-slate-900/60 dark:hover:bg-blue-950/30">
                                                    <TableCell className="font-medium text-slate-900 dark:text-slate-100">
                                                        ...
                                                        <div className="text-xs text-muted-foreground">
                                                            {t('drivers.assignmentPerformances.table.duration', { value: formatDaysLabel(row.trip_duration_days, 0) })}
                                                        </div>
                                                    </TableCell>
                                                    <TableCell>
                                                        <div className="text-sm font-medium text-slate-900 dark:text-slate-100">{route}</div>
                                                        {row.operation?.number && (
                                                            <div className="text-xs text-muted-foreground">{t('drivers.assignmentPerformances.table.operation', { value: row.operation.number })}</div>
                                                        )}
                                                    </TableCell>
                                                    <TableCell className="text-right text-sm font-semibold">
                                                        {formatKilometers(row.total_distance_km, 1, notAvailableLabel)}
                                                        <div className="text-xs text-muted-foreground">
                                                            {t('drivers.assignmentPerformances.table.loadedDistance', { value: formatKilometers(row.DistanceWCargo, 1, notAvailableLabel) })}
                                                        </div>
                                                    </TableCell>
                                                    <TableCell className="text-right text-sm font-semibold">
                                                    ...
                                                    <TableCell className="text-sm">
                                                        <div className="flex flex-col gap-2">
                                                            <Badge className={statusTone}>
                                                                {row.is_returned
                                                                    ? t('drivers.assignmentPerformances.table.returned')
                                                                    : t('drivers.assignmentPerformances.table.inTransit')}
                                                            </Badge>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <Button variant="ghost" size="sm" asChild>
                                                            <Link href={`/performances/${row.id}`} className="inline-flex items-center gap-1">
                                                                {t('drivers.assignmentPerformances.table.view')}
                            ...
                            <div className="py-12 text-center text-muted-foreground">
                                ...
                                <p className="text-lg font-semibold text-slate-900 dark:text-slate-100">{t('drivers.assignmentPerformances.empty.title')}</p>
                                <p className="mt-2 text-sm">{t('drivers.assignmentPerformances.empty.description')}</p>
                            </div>
                        )}
