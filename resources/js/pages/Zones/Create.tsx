...
export default function ZonesCreate({ regions }: ZoneCreateProps) {
  const { toast } = useToast()
  const [frontendErrors, setFrontendErrors] = useState<Record<string, string>>({})
  const { data, setData, post, processing, errors } = useForm<ZoneFormData>({
    name: '',
    region_id: '',
    status: 'active',
    code: '',
    ...
    <div className="flex h-full flex-1 flex-col gap-6 overflow-auto p-4">
      ...
      <Card className="max-w-2xl">
        <CardHeader className="border-b">
          <h2 className="text-lg font-semibold">New Zone Details</h2>
        </CardHeader>
...
ZonesCreate.layout = (page: React.ReactNode) => <AppLayout children={page} />
