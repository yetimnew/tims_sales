<?php

namespace Database\Seeders;

use App\Models\NotificationType;
use Illuminate\Database\Seeder;

class NotificationTypeSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $types = [
            [
                'key' => NotificationType::TRUCK_CREATED,
                'name' => 'Truck Created',
                'description' => 'Triggered when a new truck is added to the system.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::TRUCK_UPDATED,
                'name' => 'Truck Updated',
                'description' => 'Triggered when a truck record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::TRUCK_DELETED,
                'name' => 'Truck Deleted',
                'description' => 'Triggered when a truck is removed from the system.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::USER_CREATED,
                'name' => 'User Created',
                'description' => 'Triggered when an account is created.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::USER_UPDATED,
                'name' => 'User Updated',
                'description' => 'Triggered when an account profile changes.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::USER_DELETED,
                'name' => 'User Deleted',
                'description' => 'Triggered when an account is deleted.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::ROLE_CREATED,
                'name' => 'Role Created',
                'description' => 'Triggered when a role is created.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::ROLE_UPDATED,
                'name' => 'Role Updated',
                'description' => 'Triggered when a role changes.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::ROLE_DELETED,
                'name' => 'Role Deleted',
                'description' => 'Triggered when a role is deleted.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::DRIVER_CREATED,
                'name' => 'Driver Created',
                'description' => 'Triggered when a driver is added to the system.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_UPDATED,
                'name' => 'Driver Updated',
                'description' => 'Triggered when a driver record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_DELETED,
                'name' => 'Driver Deleted',
                'description' => 'Triggered when a driver is removed from the system.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::DRIVER_STATUS_UPDATED,
                'name' => 'Driver Status Updated',
                'description' => 'Triggered when a driver updates status from the mobile app.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::EMERGENCY_ALERT_CREATED,
                'name' => 'Emergency Alert Created',
                'description' => 'Triggered when a driver sends an emergency alert from the mobile app.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::MAINTENANCE_ASSIGNED,
                'name' => 'Maintenance Assigned',
                'description' => 'Triggered when maintenance is scheduled for a driver’s assigned truck.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_DUE_SOON,
                'name' => 'Maintenance Due Soon',
                'description' => 'Triggered when assigned-truck maintenance is due soon.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_OVERDUE,
                'name' => 'Maintenance Overdue',
                'description' => 'Triggered when assigned-truck maintenance becomes overdue.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_DRIVER_ACKNOWLEDGED,
                'name' => 'Maintenance Driver Acknowledged',
                'description' => 'Triggered when a driver acknowledges a maintenance alert.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_ISSUE_REPORTED,
                'name' => 'Maintenance Issue Reported',
                'description' => 'Triggered when a driver reports a maintenance issue from mobile.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_SERVICE_REQUESTED,
                'name' => 'Maintenance Service Requested',
                'description' => 'Triggered when a driver requests maintenance service from mobile.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_REQUEST_APPROVED,
                'name' => 'Maintenance Request Approved',
                'description' => 'Triggered when a manager or dispatcher approves a mobile maintenance request.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::MAINTENANCE_REQUEST_REJECTED,
                'name' => 'Maintenance Request Rejected',
                'description' => 'Triggered when a manager or dispatcher rejects a mobile maintenance request.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_TRUCK_CREATED,
                'name' => 'Driver-Truck Assignment Created',
                'description' => 'Triggered when a driver is assigned to a truck.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_TRUCK_UPDATED,
                'name' => 'Driver-Truck Assignment Updated',
                'description' => 'Triggered when a driver-truck assignment changes.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_TRUCK_DELETED,
                'name' => 'Driver-Truck Assignment Deleted',
                'description' => 'Triggered when a driver-truck assignment is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::DAILY_TRUCK_STATUS_CREATED,
                'name' => 'Daily Truck Status Created',
                'description' => 'Triggered when a truck receives a status assignment for a given day.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DAILY_TRUCK_STATUS_UPDATED,
                'name' => 'Daily Truck Status Updated',
                'description' => 'Triggered when a truck status assignment changes.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DAILY_TRUCK_STATUS_DELETED,
                'name' => 'Daily Truck Status Deleted',
                'description' => 'Triggered when a truck status assignment is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::VEHICLE_TYPE_CREATED,
                'name' => 'Vehicle Type Created',
                'description' => 'Triggered when a vehicle type is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::VEHICLE_TYPE_UPDATED,
                'name' => 'Vehicle Type Updated',
                'description' => 'Triggered when a vehicle type is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::VEHICLE_TYPE_DELETED,
                'name' => 'Vehicle Type Deleted',
                'description' => 'Triggered when a vehicle type is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::FUEL_RECORD_CREATED,
                'name' => 'Fuel Record Created',
                'description' => 'Triggered when a fuel record is created.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::FUEL_RECORD_UPDATED,
                'name' => 'Fuel Record Updated',
                'description' => 'Triggered when a fuel record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::FUEL_RECORD_DELETED,
                'name' => 'Fuel Record Deleted',
                'description' => 'Triggered when a fuel record is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::DRIVER_SAFETY_CREATED,
                'name' => 'Driver Safety Record Created',
                'description' => 'Triggered when a safety record is logged for a driver.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_SAFETY_UPDATED,
                'name' => 'Driver Safety Record Updated',
                'description' => 'Triggered when a driver safety record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DRIVER_SAFETY_DELETED,
                'name' => 'Driver Safety Record Deleted',
                'description' => 'Triggered when a driver safety record is deleted.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::CARGO_TYPE_CREATED,
                'name' => 'Cargo Type Created',
                'description' => 'Triggered when a cargo type is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::CARGO_TYPE_UPDATED,
                'name' => 'Cargo Type Updated',
                'description' => 'Triggered when a cargo type is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::CARGO_TYPE_DELETED,
                'name' => 'Cargo Type Deleted',
                'description' => 'Triggered when a cargo type is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::STATUS_TYPE_CREATED,
                'name' => 'Status Type Created',
                'description' => 'Triggered when a status type is created.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::STATUS_TYPE_UPDATED,
                'name' => 'Status Type Updated',
                'description' => 'Triggered when a status type changes.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::STATUS_TYPE_DELETED,
                'name' => 'Status Type Deleted',
                'description' => 'Triggered when a status type is deleted.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::REGION_CREATED,
                'name' => 'Region Created',
                'description' => 'Triggered when a region is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::REGION_UPDATED,
                'name' => 'Region Updated',
                'description' => 'Triggered when a region record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::REGION_DELETED,
                'name' => 'Region Deleted',
                'description' => 'Triggered when a region is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::ZONE_CREATED,
                'name' => 'Zone Created',
                'description' => 'Triggered when a zone is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::ZONE_UPDATED,
                'name' => 'Zone Updated',
                'description' => 'Triggered when a zone record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::ZONE_DELETED,
                'name' => 'Zone Deleted',
                'description' => 'Triggered when a zone is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::WOREDA_CREATED,
                'name' => 'Woreda Created',
                'description' => 'Triggered when a woreda is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::WOREDA_UPDATED,
                'name' => 'Woreda Updated',
                'description' => 'Triggered when a woreda record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::WOREDA_DELETED,
                'name' => 'Woreda Deleted',
                'description' => 'Triggered when a woreda is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::PLACE_CREATED,
                'name' => 'Place Created',
                'description' => 'Triggered when a place is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::PLACE_UPDATED,
                'name' => 'Place Updated',
                'description' => 'Triggered when a place record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::PLACE_DELETED,
                'name' => 'Place Deleted',
                'description' => 'Triggered when a place is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::DISTANCE_CREATED,
                'name' => 'Distance Created',
                'description' => 'Triggered when a distance record is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DISTANCE_UPDATED,
                'name' => 'Distance Updated',
                'description' => 'Triggered when a distance record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::DISTANCE_DELETED,
                'name' => 'Distance Deleted',
                'description' => 'Triggered when a distance record is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::OUTSOURCE_CREATED,
                'name' => 'Outsource Created',
                'description' => 'Triggered when an outsource vendor is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::OUTSOURCE_UPDATED,
                'name' => 'Outsource Updated',
                'description' => 'Triggered when an outsource vendor is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::OUTSOURCE_DELETED,
                'name' => 'Outsource Deleted',
                'description' => 'Triggered when an outsource vendor is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::OUTSOURCE_PERFORMANCE_CREATED,
                'name' => 'Outsource Performance Created',
                'description' => 'Triggered when an outsource performance record is added.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::OUTSOURCE_PERFORMANCE_UPDATED,
                'name' => 'Outsource Performance Updated',
                'description' => 'Triggered when an outsource performance record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::OUTSOURCE_PERFORMANCE_DELETED,
                'name' => 'Outsource Performance Deleted',
                'description' => 'Triggered when an outsource performance record is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::OPERATION_CREATED,
                'name' => 'Operation Created',
                'description' => 'Triggered when an operation is created.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::OPERATION_UPDATED,
                'name' => 'Operation Updated',
                'description' => 'Triggered when an operation is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::OPERATION_DELETED,
                'name' => 'Operation Deleted',
                'description' => 'Triggered when an operation is deleted.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::CUSTOMER_CREATED,
                'name' => 'Customer Created',
                'description' => 'Triggered when a customer is created.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::CUSTOMER_UPDATED,
                'name' => 'Customer Updated',
                'description' => 'Triggered when a customer record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::CUSTOMER_DELETED,
                'name' => 'Customer Deleted',
                'description' => 'Triggered when a customer is removed.',
                'default_in_app' => true,
                'default_email' => true,
            ],
            [
                'key' => NotificationType::PERFORMANCE_CREATED,
                'name' => 'Performance Created',
                'description' => 'Triggered when a performance record is created.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::PERFORMANCE_UPDATED,
                'name' => 'Performance Updated',
                'description' => 'Triggered when a performance record is updated.',
                'default_in_app' => true,
                'default_email' => false,
            ],
            [
                'key' => NotificationType::PERFORMANCE_DELETED,
                'name' => 'Performance Deleted',
                'description' => 'Triggered when a performance record is deleted.',
                'default_in_app' => true,
                'default_email' => true,
            ],
        ];

        NotificationType::query()->upsert(
            $types,
            ['key'],
            ['name', 'description', 'default_in_app', 'default_email']
        );
    }
}
